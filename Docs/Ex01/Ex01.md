...
1. In the search results, select **Prompt flow for VS Code** and then select **Install**. Wait while the extension installs.
    ...
    >{: .warning } You may see an error stating that the extension is not compatbile with the version of Visual Studio Code that you have installed. If you see the error, follow the steps in Exercise 01 Task 02 to upgrade Visual Studio Code.
...
1. Enter the following command at the Visual Studio Code Terminal window prompt. This command returns the Docker version if Docker is installed. 
    ...
    >{: .warning } If Docker is installed, skip the remaining steps in this task and move to the next exercise. Otherwise, complete the following steps to install Docker.
